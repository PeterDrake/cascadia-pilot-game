 ...
 * @default ====================================
 *
 * @param Game Reconstruction (1.5.2 & below)
 * @type boolean
 * @desc If using MV 1.5.2 or below, this is required for all "Game Window" parameters. For MV 1.6.0+, it does nothing.
 * @default true
 * @parent Game Window
 *
...
Imported["SumRndmDde Game Upgrade"] = 1.30;
		...
		if(typeof json["window"] === "object") {
			...
			for(let i = 0; i < keys.length; i++) {
				if(keys[i] === "height") {
					...
					console.log(json["window"]["height"] - offset);
					console.log(_.windowSettings["height"])
...
window.onload = function() {
	if(!_.isNewNWjs && _.params['Game Reconstruction'] && GameWindowManager.isWindowOriginal()) {
