...
if (back_buttons) {
    ...
    Window_ItemCategory.prototype.makeCommandList = function() {
        // Detect if we are inside a shop or synthesis menu, If yes, don't add "Back...":
        var is_shop = false;
        var windows = AdvancedMenus.listOfWindows();
        for (var j = 0; j < windows.length; j++) {
            if (Window_ShopCommand.prototype.isPrototypeOf(windows[j]) || Window_SynthesisCommand.prototype.isPrototypeOf(windows[j])) {
                is_shop = true;
                break;
            }
        }
...
// Allow changing game menu category while Window_SynthesisStatus selection active:
Window_SynthesisCommand.prototype._activateWhenOutOfFocus = function() {
       var windows = AdvancedMenus.listOfWindows();
    // exit if synthesis list is open
    for (var j = 0; j < windows.length; j++) {
        if (Window_SynthesisList.prototype.isPrototypeOf(windows[j]) &&
                windows[j].active && windows[j].visible) {
            windows[j].processCancel();
           return true;
        }
    }
    // exit if synthesis ingredients is open
    for (var j = 0; j < windows.length; j++) {
        if (Window_SynthesisIngredients.prototype.isPrototypeOf(windows[j]) &&
                windows[j].active && windows[j].visible) {
            windows[j].processCancel();
            return true;
        }
    }
    // exit if synthesis number is selected
    for (var j = 0; j < windows.length; j++) {
        if (Window_SynthesisNumber.prototype.isPrototypeOf(windows[j]) &&
                windows[j].active && windows[j].visible) {
            windows[j].processCancel();
            return true;
        }
    }
    // exit synthesis status
    for (var j = 0; j < windows.length; j++) {
        if (Window_SynthesisStatus.prototype.isPrototypeOf(windows[j]) &&
                windows[j].active && windows[j].visible) {
            windows[j].processCancel();
            return true;
        }
